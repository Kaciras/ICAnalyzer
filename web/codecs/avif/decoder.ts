 ...
 */
import type { AVIFModule } from "squoosh/codecs/avif/dec/avif_dec";
import wasmUrl from "squoosh/codecs/avif/dec/avif_dec.wasm";
...
export async function decode(data: ArrayBuffer): Promise<ImageData> {
	if (!emscriptenModule) {
		const decoder = await import("squoosh/codecs/avif/dec/avif_dec");
